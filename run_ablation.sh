...
python train_single_step.py --data $DATA_PATH --num_nodes $NODES --epochs $EPOCHS --runs $RUNS --device $DEVICE --save ./model/model_M2.pt --revin 1 --dual_graph 1 --use_router 0 --use_dirloss 0 --adj_data ./data/sensor_graph/adj_mx.pkl > log_M2_dualgraph.txt 2>&1
echo "✅ M2 运行完毕，日志已保存至 log_M2_dualgraph.txt"

# 4. 跑 M3 (+ RevIN + 静态双图 + 动态路由)
echo "[4/5] 正在运行 M3: MTGNN + RevIN + Dual Graph + MoE Router..."
python train_single_step.py --data $DATA_PATH --num_nodes $NODES --epochs $EPOCHS --runs $RUNS --device $DEVICE --save ./model/model_M3.pt --revin 1 --dual_graph 1 --use_router 1 --use_dirloss 0 --adj_data ./data/sensor_graph/adj_mx.pkl > log_M3_router.txt 2>&1
